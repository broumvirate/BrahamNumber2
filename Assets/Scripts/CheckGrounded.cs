...
public class CheckGrounded : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        RaycastHit2D groundCheck = Physics2D.Raycast(new Vector2(GetComponent<BoxCollider2D>().bounds.center.x - (GetComponent<BoxCollider2D>().bounds.extents.x - 0.1f), GetComponent<BoxCollider2D>().bounds.center.y), Vector2.down, distToGround + 0.1f, groundLayer);
        RaycastHit2D groundCheck2 = Physics2D.Raycast(new Vector2(GetComponent<BoxCollider2D>().bounds.center.x + (GetComponent<BoxCollider2D>().bounds.extents.x - 0.1f), GetComponent<BoxCollider2D>().bounds.center.y), Vector2.down, distToGround + 0.1f, groundLayer);
